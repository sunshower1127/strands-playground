...
### 단일 에이전트 vs 멀티 에이전트 프롬프트 구성 (2025 연구)

#### 핵심 결론
최신 LLM(GPT-o3, Gemini 2.5 Pro 등)의 성능이 크게 향상되면서 **멀티 에이전트의 이점이 상당히 줄어듦**.

#### 성능 비교 수치
| 항목 | ChatGPT 기준 | Gemini 2.0 Flash 기준 |
|-----|-------------|---------------------|
| MAS 성능 향상 | +10% | **+3%** (축소) |
| HumanEval 향상 | +10.7% | **+3.0%** |

**효율성 차이:**
- 멀티 에이전트는 **입력 토큰 4~220배** 더 소비
- **출력 토큰 2~12배** 더 필요

#### 단일 에이전트 (프롬프트 몰아주기)
| 장점 | 단점 |
|-----|-----|
| 낮은 비용 & 낮은 지연시간 | 선형적 워크플로우에 제한 |
| 디버깅 용이 | 단일 실패점 |
| 통합된 컨텍스트 (전체 정보 접근) | |
| 정보 손실 없음 | |
...
| 장점 | 단점 |
|-----|-----|
| 복잡한 문제 해결력 (AIME 등) | 비용 급증 (에이전트마다 LLM 호출) |
| 역할 전문화 | 지연시간 증가 (3-agent = 3배) |
| 모듈화 & 확장성 | Overthinking 문제 |
| 실패 복원력 | 정보 손실 → 오류 전파 |

#### 권장 적용 방식
| 상황 | 권장 방식 |
|-----|---------|
| 단순 RAG, 요약, 질의응답 | **단일 에이전트** |
| 명확한 단일 태스크 | **단일 에이전트** |
| 복잡한 다단계 워크플로우 | **멀티 에이전트** |
| 여러 도메인 교차 작업 | **멀티 에이전트** |

**하이브리드 접근법:**
- **에이전트 라우팅**: 복잡도 평가 후 SAS/MAS 선택 → 정확도 2%↑, 비용 50%↓
- **에이전트 캐스케이드**: 단일 먼저 시도, 실패 시 멀티 → 정확도 12%↑, 비용 20%↓

**참고:**
- [Single-agent or Multi-agent Systems? Why Not Both? (arXiv, 2025)](https://arxiv.org/html/2505.18286v1)
...
**Q: 하나의 프롬프트에 여러 언어를 섞어 써도 되나?**

**A: 네, 섞어 쓰는 것이 정답입니다.**

LLM은 다국어 혼합(Code-switching)을 잘 처리합니다. 오히려 **역할 분리**가 명확해집니다:
- 영어 = "이건 시스템 지시야" (규칙, 제약)
- 한국어 = "이건 사용자 맥락이야" (질문, 문서)
...
│  "You MUST respond in Korean. This is mandatory."        │
├─────────────────────────────────────────────────────────┤
│  User Prompt (한국어)                                    │
│  "## 참고 문서"                                          │
│  {context} ← 원본 언어 그대로                            │
...
→ LLM은 영어 지시를 이해하고, 한국어 컨텍스트를 처리하고, 한국어로 응답합니다.
